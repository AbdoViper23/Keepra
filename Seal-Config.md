...
- **No SLA on testnet.** The Pricing page warns: testnet key servers have no availability guarantees, SLAs, or long-term key persistence. Fine for the hackathon; not for mainnet production.
...
- **Mainnet.** When migrating to mainnet (post-submission, see Roadmap v1), all of these values will be different. Don't reuse testnet IDs on mainnet.
- **Re-verify before submission.** Check the live Pricing page (https://seal-docs.wal.app/Pricing) the day before hackathon submission to confirm values haven't changed.
